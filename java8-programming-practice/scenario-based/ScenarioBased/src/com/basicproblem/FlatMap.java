...
public class FlatMap {
	public static void main(String[] args) {
		...
		students.stream().flatMap(x -> Stream.of(x.contact)).forEach(System.out::println);;
		students.stream().flatMap(x -> Stream.of(x.contact));
		

		// Count the total number of contact numbers.
		System.out.println("---------------------------------------->");
		System.out.println(students.stream().flatMap(x -> x.contact.stream()).collect(Collectors.groupingBy(x -> x)));;

		// Find students who have more than one contact number.
