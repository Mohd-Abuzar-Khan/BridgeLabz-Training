...
public class FilteringAndMapping {
	public static void main(String[] args) {
		...
		// Print all students whose rank is less than 50.
		Stream<Student> student = students.stream();
		List<Student> lessThan50 = student.filter((x) -> x.rank < 50).collect(Collectors.toList());
		lessThan50.forEach(Student::printDetails);
		
		
		// Print all students whose age is greater than 25.
		System.out.println("-------------------------------------->");
		List<Student> greaterThan25 = students.stream().filter((x) -> x.age > 25).collect(Collectors.toList());
		greaterThan25.forEach(Student::printDetails);
		
		//Print the names of all students only
		System.out.println("-------------------------------------->");
		System.out.println(students.stream().map(x -> x.name).collect(Collectors.toList()));
//		List<Student> names = students.stream().map(x -> x.name).collect(Collectors.toList());
//		names.forEach(Student::printDetails);
		
		
		// Print all students not from Dehli.
		System.out.println("-------------------------------------->");
		List<Student> dehliStudent = students.stream().filter((x) -> x.city == "Delhi").collect(Collectors.toList());
		dehliStudent.forEach(Student::printDetails);
		
		// Print all students not from Mumbai.
		System.out.println("-------------------------------------->");
		List<Student> mumbaiStudent = students.stream().filter((x) -> x.city == "Mumbai").collect(Collectors.toList());
		mumbaiStudent.forEach(Student::printDetails);
	}
}
