...
public class Report {
	...
	static Transactions[] sellList = new Transactions[11];
	static int index = 0;

	// Method to add transaction
	static void addTransaction(LocalDate date, double amount) {
		Transactions sell = new Transactions(date, amount);
	...
	public static void mergeSort(Transactions[] arr, int l, int r) {
		if (l < r) {
	...
	private static void merge(Transactions[] arr, int l, int m, int r) {
		// Find sizes of two sub arrays to be merged
		int n1 = m - l + 1;
		int n2 = r - m;

		// Create temp arrays
		Transactions[] L = new Transactions[n1];
		Transactions[] R = new Transactions[n2];
	...
	public static void sortAmount(Transactions[] arr) {
		int i = 0;
		int j = 0;
		while (j < arr.length) {
	...
	public static void mergeSortAmount(Transactions[] arr, int l, int r) {
		if (l < r) {
			// Find the middle point
			int m = (l + r) / 2;
	...
	private static void mergeAmount(Transactions[] arr, int l, int m, int r) {
		// Find sizes of two sub arrays to be merged
		int n1 = m - l + 1;
		int n2 = r - m;
